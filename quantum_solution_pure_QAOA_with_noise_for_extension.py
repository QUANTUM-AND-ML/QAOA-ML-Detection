...
T1 = [197.79, 158.07, 278.36, 223.29, 109.91, 236.63, 193.96,197.79, 158.07, 278.36, 223.29, 109.91]
T2 = [97.02, 48.06, 83.1, 211, 126.96, 188.44, 291.83,97.02, 48.06, 83.1, 211, 126.96]
P01 = [0.031, 0.0222, 0.0282, 0.018, 0.023, 0.0264, 0.0072,0.031, 0.0222, 0.0282, 0.018, 0.023]
P10 = [0.0226, 0.0236, 0.023, 0.013, 0.0164, 0.0208, 0.0054,0.0226, 0.0236, 0.023, 0.013, 0.0164]
gateErrors = [['rz', 0.0001836, 0.0003291, 0.0002153, 0.0002381, 0.0003141, 0.0002563, 0.0003357,0.0001836, 0.0003291, 0.0002153, 0.0002381, 0.0003141],
              ['rx', 0.0001836, 0.0003291, 0.0002153, 0.0002381, 0.0003141, 0.0002563, 0.0003357,0.0001836, 0.0003291, 0.0002153, 0.0002381, 0.0003141],
              ['h', 0.0001836, 0.0003291, 0.0002153, 0.0002381, 0.0003141, 0.0002563, 0.0003357,0.0001836, 0.0003291, 0.0002153, 0.0002381, 0.0003141],
              ['cx', 0.00735, 0.00769667, 0.00694, 0.00835, 0.01169, 0.0098133, 0.00984,0.00735, 0.00769667, 0.00694, 0.00835, 0.01169]]
...
# 获取排名前16的键值对
sorted_bitstrings = sorted(full_result_counts.items(), key=lambda x: x[1], reverse=True)[:14]
print(sorted_bitstrings)
# 反转字符串序列和0与1换位
reversed_counts = {}
for key, value in sorted_bitstrings:
    reversed_key = ''.join(['1' if char == '0' else '0' for char in key[::-1]])  # 反转键并进行0和1的取反操作
    reversed_counts[reversed_key] = value/2048

print(reversed_counts)
sorted_counts = {k: v for k, v in sorted(reversed_counts.items(), key=lambda item: int(item[0], 2))}
# 自定义 RGB 颜色
custom_color1 = mcolors.to_hex((42/255, 157/255, 142/255))  # 例如：橘色
custom_color2 = mcolors.to_hex((0.2, 0.4, 0.6))  # 例如：浅蓝色
# 使用 plot_histogram 函数，并传递 full_result_counts 和 bar_labels 参数
plot_histogram(sorted_counts, bar_labels=True, figsize=(20, 16), color= custom_color1)
plt.show()

decoded_bits1 = decoded_bits
...
# 使用 plot_histogram 函数，并传递 full_result_counts 和 bar_labels 参数
plot_histogram(reversed_counts, bar_labels=True, figsize=(20, 16), color= custom_color2)
plt.show()

decoded_bits2 = decoded_bits
print('transmitter_symbols', transmitter_symbols)
print('x', transmitter_signals)
print('y', received_signals)
print('Random + COBYLA', decoded_bits1)
print('Bayesian + COBYLA', decoded_bits2)

pylab.rcParams["figure.figsize"] = (12, 8)
pylab.plot(counts, values, label='Random + COBYLA', color= custom_color1, linestyle='-')
pylab.plot(counts1, values1, label='Bayesian + COBYLA', color=custom_color2, linestyle='--')
pylab.xlabel("Steps")
pylab.ylabel("Ground energy value")
pylab.legend(loc="upper right", fontsize='large')
plt.show()
